% NewsutterMP285 A class for using the Sutter MP-285 positioner

% 

% NewsutterMP285 implements a class for working with a Sutter MP-285
...
% >> sutter=NewsutterMP285('/dev/tty.usbserial-AE016TW4');

% NewsutterMP285: version 1.0

% NewsutterMP285: Sutter ready.
...
classdef NewsutterMP285 < serial
	...
	methods
        ...
        % return instance of NewsutterMP285 class.

        % creates a serial port object

		function obj = NewsutterMP285(varargin)
            if nargin < 1
                fprintf(1,'NewsutterMP285: ERROR you must supply a USB port name\n');
                if ispc, fprintf(1,'    e.g., "s=NewsutterMP285(''COM4'')"\n');
                else fprintf(1,'    e.g., "s=NewsutterMP285(''/dev/tty.usbXXXX'')"\n');
                end
                if ~ispc
                    disp(ls('/dev/tty.*'))
                end
            ...
            set(obj, 'Tag','NewsutterMP285');            
            set(obj,'Timeout',60);
            set(obj,'Tag','NewsutterMP285');
            if obj.verbose >= 1, fprintf(1,'NewsutterMP285: %s\n',obj.versionStr); end
            ...
            if currentVelocity==777
                fprintf(1,'NewsutterMP285: Sutter ready.\n');
                setVelocity(obj, 1000, 10);
                updatePanel(obj);
            else
                fprintf(1,'NewsutterMP285: WARNING Sutter did not respond at startup.\n');
            end
        end
        ...
        function [stepMult, currentVelocity, vScaleFactor]=getStatus(obj)
            ...
            if obj.verbose >= 2, fprintf(1,'NewsutterMP285: get status info:\n'); end
        ...
        function xyz_um=getPosition(obj)
            ...
            if obj.verbose >= 2
                ...
                fprintf(1,'NewsutterMP285: Stage Position:\n');
        ...
        %  complete the move.
            ...
            if obj.verbose >= 2, fprintf(1,'NewsutterMP285: Sutter translating %g %g %g um...\n',xyz); end
        ...
        % stepMult queried in status, should be 25 steps/um step resolution = 0.04 um
            ...
            %     fprintf(1,'NewsutterMP285: WARNING: movement restricted to travel limits %d,%d um\n',posLimit,negLimit);
            ...
            if obj.verbose >= 2, fprintf(1,'NewsutterMP285: Sutter moving to %g %g %g um...\n',xyz); end
            ...
            if isempty(cr)
                ...
                if obj.verbose >= 2, fprintf(1,'NewsutterMP285: Sutter move complete. (%.2f sec)\n',moveTime); end
