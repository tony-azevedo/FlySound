function data = runProtocol(exp_info)
...
data(n).recMode = readMode;                             % recording mode ('VClamp' or 'IClamp')
data(n).recGain = readGain;                             % alpha on the amp
data(n).headstageGain = 1;                             % Change this if changing \beta on the amp

aiSession = exp_info.aiSession; aiSession = exp_info.aoSession;
